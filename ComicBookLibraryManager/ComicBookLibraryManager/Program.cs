...
namespace ComicBookLibraryManager
{
    internal class Program
    {
        ...
        private const string COMMAND_LIST_COMIC_BOOKS = "l";
        private const string COMMAND_LIST_COMIC_BOOK = "i";
        private const string COMMAND_LIST_COMIC_BOOK_PROPERTIES = "p";
        private const string COMMAND_ADD_COMIC_BOOK = "a";
        private const string COMMAND_UPDATE_COMIC_BOOK = "u";
        private const string COMMAND_DELETE_COMIC_BOOK = "d";
        private const string COMMAND_SAVE = "s";
        private const string COMMAND_CANCEL = "c";
        ...
        {
            var command = COMMAND_LIST_COMIC_BOOKS;
            ...
            while (command != COMMAND_QUIT)
            {
                switch (command)
                {
                    case COMMAND_LIST_COMIC_BOOKS:
                        comicBookIds = ListComicBooks();
                        break;
                    case COMMAND_ADD_COMIC_BOOK:
                        AddComicBook();
                        command = COMMAND_LIST_COMIC_BOOKS;
                        continue;
                    default:
        ...
        private static int GetSeriesId()
        {
            ...
            while (seriesId == null)
            {
                ...
                var lineNumberInput = ConsoleHelper.ReadInput(
        ...
        private static int GetArtistId()
        {
            ...
            while (artistId == null)
            {
                ...
                {
                    ConsoleHelper.OutputLine("{0}) {1}", artists.IndexOf(artist) + 1, artist.Name);
                }
        ...
        {
            string command = COMMAND_LIST_COMIC_BOOK;

            // If the current command doesn't equal the "Cancel" command 
            // then evaluate and process the command.
            while (command != COMMAND_CANCEL)
            {
                switch (command)
                {
                    case COMMAND_LIST_COMIC_BOOK:
                        ListComicBook(comicBookId);
                        break;
                    case COMMAND_UPDATE_COMIC_BOOK:
                        UpdateComicBook(comicBookId);
                        command = COMMAND_LIST_COMIC_BOOK;
                        continue;
                    case COMMAND_DELETE_COMIC_BOOK:
                        command = DeleteComicBook(comicBookId) ? COMMAND_CANCEL : COMMAND_LIST_COMIC_BOOK;
                        continue;
                    default:
                        ConsoleHelper.OutputLine("Sorry, but I didn't understand that command.");
                        break;
        ...
        private static void UpdateComicBook(int comicBookId)
        {
            ...
            string command = COMMAND_LIST_COMIC_BOOK_PROPERTIES;
            ...
            while (command != COMMAND_CANCEL)
            {
                switch (command)
                {
                    case COMMAND_LIST_COMIC_BOOK_PROPERTIES:
                    ...
                    default:
                        ...
                        {
                            command = COMMAND_LIST_COMIC_BOOK_PROPERTIES;
                            continue;
                        }
                        else
                        {
            ...
            string command, ComicBook comicBook)
        {
            ...
            if (lineNumber > 0 && lineNumber <= EditableProperties.Count)
            {
                ...
                var propertyName = EditableProperties[lineNumber - 1];
